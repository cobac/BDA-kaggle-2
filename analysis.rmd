...
```{r}
analysis_df <- analysis_df %>%
  select(-c(user_id, exp_id, epoch, sampleid, n_samples))
...
- TODO: highly correlated variables, variables without variance
...
```{r}
model_lca <- MASS::lda(activity ~ ., data = analysis_df) 
```

## Model 4 - Something knn
